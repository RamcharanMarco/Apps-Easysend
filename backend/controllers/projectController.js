...
const updateProject = async (req, res) =>{
    ...
    const {name,description,repo,demo,type,lib,lang,db,css,hosting} = req.body
    console.log(name,description,repo,demo,type,lib,lang,db,css,hosting)
    try{
     const project = await Project.findByIdAndUpdate(_id,{name,description,repo,demo,type,lib,lang,db,css,hosting})
        res.status(200).json(project)
    }catch(error){
        res.status(400).json({error : error.message})
    }
